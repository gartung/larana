...
pid::NNReader::NNReader(const char* xmlFileName) :
	_invH(0), _N01(0)
{
...
void pid::NNReader::SetupNetwork(const char* xmlFile)
{
...
unsigned int pid::NNReader::NetworkSize(void) const
{
...
void pid::NNReader::Run(const TVectorT<float>& vInput)
{
...
void pid::NNReader::Run(const std::vector<float>& vInput)
{
...
void pid::NNReader::Run(const std::vector<double>& vInput)
{
	if (vInput.size() == _nInps)
	{
		((LayerInp*)_layers[0])->Activate(vInput);
...
float pid::NNReader::GetOneOutput(int neuronIndex) const
{
	if (neuronIndex < _nOuts) return _layers.back()->GetOutput()[neuronIndex];
	else throw "OutSizeErr";
}

TVectorT<float> pid::NNReader::GetAllOutputs() const
{
...
TVectorT<float> pid::NNReader::GetLinOutputs() const
{
	TVectorT<float> vret(_nOuts);
	LayerSoftmax* softmax = dynamic_cast< LayerSoftmax* >(_layers.back());
...
float pid::NNReader::OutputSigma2(unsigned int outIndex)
{
...
TMatrixT<float> pid::NNReader::OutputCov(void)
{
...
void pid::NNReader::HiddenGradient(unsigned int layerIndex)
{
...
void pid::NNReader::ClearGradient(void)
{
	for (unsigned int l = 1; l < _layers.size(); l++) _layers[l]->ClearGradient();
}

void pid::NNReader::F_dLin_dw(TMatrixT<float>& vDst, size_t row, unsigned int outIndex)
{
	ClearGradient();
...
void pid::NNReader::GradientToVector(TMatrixT<float>& vDst, size_t row) const
{
...
void pid::NNReader::F_dLin_dw(TVectorT<float>& vDst, unsigned int outIndex)
{
	ClearGradient();
...
void pid::NNReader::GradientToVector(TVectorT<float>& vDst) const
{
	for (size_t l = 1, pos = 0; l < _layers.size(); l++)
		for (size_t n = 0; n < _layers[l]->NLength(); n++)
			for (int c = 0; c < _layers[l]->Gradient(n).GetNrows(); c++)
...
pid::NNReader::Layer::Layer(int nNeurons, Layer* prevLayer) :
	_type(pid::NNReader::kLayerBase),
	_nLength(nNeurons),
...
TVectorT<float> pid::NNReader::Layer::ActGradient(unsigned int nIndex) const
{
...
void pid::NNReader::Layer::ClearGradient(void)
{
...
float pid::NNReader::LayerSig::f_f[F_TABLE_LENGTH];
float pid::NNReader::LayerSig::f_d[F_TABLE_LENGTH];
float pid::NNReader::LayerSig::f_factor;

pid::NNReader::LayerSig::LayerSig(int nNeurons, Layer* prevLayer) :
	Layer(nNeurons, prevLayer)
{
	_type = pid::NNReader::kLayerSig;
...
float pid::NNReader::LayerSig::ActDerivative(unsigned int nIndex) const
{
	float fValue = _output[nIndex];
	return fValue * (1.0F - fValue);
}

float pid::NNReader::LayerSig::_fsig(double dotprod) const
{
	int i;
	float xd;
...
void pid::NNReader::LayerSig::Activate()
{
...
float pid::NNReader::LayerTanh::f_f[F_TABLE_LENGTH];
float pid::NNReader::LayerTanh::f_d[F_TABLE_LENGTH];
float pid::NNReader::LayerTanh::f_factor;

pid::NNReader::LayerTanh::LayerTanh(int nNeurons, Layer* prevLayer) :
	Layer(nNeurons, prevLayer)
{
	_type = pid::NNReader::kLayerTanh;
	f_factor = (float)(F_TABLE_LENGTH - 1) / F_TABLE_MAX;
	for (unsigned int i = 0; i < F_TABLE_LENGTH; i++)
	{
...
float pid::NNReader::LayerTanh::ActDerivative(unsigned int nIndex) const
{
	float fValue = _output[nIndex];
	return 1.0F - fValue * fValue;
}

float pid::NNReader::LayerTanh::_ftanh(double dotprod) const
{
	int i;
	float xd;
...
void pid::NNReader::LayerTanh::Activate()
{
	for (int i = 0; i < _nLength; i++)
...
pid::NNReader::LayerSoftmax::LayerSoftmax(int nNeurons, Layer* prevLayer) :
	Layer(nNeurons, prevLayer),
	_linout(nNeurons)
{
	_type = pid::NNReader::kLayerSoftmax;
}

float pid::NNReader::LayerSoftmax::ActFunction(float xValue) const
{
	if (xValue < 30.0F) return (float)exp(xValue);
	else return (float)exp(30.0);
}

TVectorT<float> pid::NNReader::LayerSoftmax::ActGradient(unsigned int nIndex) const
{
	float fValue = _output[nIndex];
	TVectorT<float> gradient(_nLength);
...
void pid::NNReader::LayerSoftmax::Activate()
{
	double o, sum = 0.0;
	for (int i = 0; i < _nLength; i++)
	{
...
void pid::NNReader::LayerSoftmax::ModerateOutputs(const TMatrixT<float>& S, unsigned int mcSize)
{
...
void pid::NNReader::LayerSoftmax::ModerateOutputs(const TMatrixT<float>& S, const TMatrixT<float>& N01)
{
...
void pid::NNReader::LayerLin::Activate()
{
	_type = pid::NNReader::kLayerLin;
	for (int i = 0; i < _nLength; i++)
		_output[i] = (float)(_previous->GetOutput() * _coeffs[i]);
}

///   LayerInp Class (nested in NNReader)   ///
pid::NNReader::LayerInp::LayerInp(int nInps) :
	_normalize(false)
{
	_type = pid::NNReader::kLayerInp;
	_nLength = nInps;
	_nInputs = nInps;
...
pid::NNReader::LayerInp::LayerInp(int nInps, const TVectorT<float>& vMean, const TVectorT<float>& vStd) :
	_normalize(true)
{
...
pid::NNReader::LayerInp::~LayerInp()
{
	if (_normalize)
	{
		delete _mean;
...
void pid::NNReader::LayerInp::Activate()
{
	_output = 0.0F;
}

void pid::NNReader::LayerInp::Normalize()
{
...
void pid::NNReader::LayerInp::Activate(const TVectorT<float>& vInput)
{
	for (int i = 0; i < vInput.GetNrows(); ++i) _output[i] = vInput[i];
	if (_normalize) Normalize();
}

void pid::NNReader::LayerInp::Activate(const std::vector<float>& vInput)
{
	for (unsigned int i = 0; i < vInput.size(); i++) _output[i] = vInput[i];
	if (_normalize) Normalize();
}

void pid::NNReader::LayerInp::Activate(const std::vector<double>& vInput)
{
